...
void matrixInversa( Matrix entrante, Matrix &inversa){
    //La Inversa de una Matriz solo es posible de calcular cuando el determinante es
    //distinto de cero.
    Matrix cofactores, traspose;
    if(determinant(entrante) != 0){
        //En el repositorio, cuenta con las siguientes funciones:
        // Determinante-- se usa arriba
        // Traspuesta
        // Cofactores
        //La Matriz Adjunta es la matriz de cofactores traspuesta:
        cofactors(entrante, cofactores);//matriz de cofactores
        //transpose(matrixtarea,traspuesta);//mastriz de cofactores traspuesta, igual a adjunta
        transpose(cofactores, traspose); //mastriz de cofactores traspuesta, igual a adjunta
        productRealMatrix(1/determinant(entrante), traspose, inversa);
    }
    else{
        // Si el determinante es igual a cero, deberá terminar su programa utilizando la macro
        //EXIT_FAILURE de la librería “stdlib.h”
        exit(EXIT_FAILURE);
    }
}